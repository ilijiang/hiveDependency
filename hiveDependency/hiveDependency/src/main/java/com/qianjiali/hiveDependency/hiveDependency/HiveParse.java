...
public class HiveParse {
	    ...
	    //hdfs地址
	    private static String hdfsConfig = "http://s1-hadoop-test.esf.fdd:8090/hadoop-config/hdfs-site.xml,http://s1-hadoop-test.esf.fdd:8090/hadoop-config/core-site.xml";
	    private static String hdfsDir = "hdfs://s1-hadoop-test.esf.fdd:8020/user/qianjiali/edw_houses/ADM/DDL/edw_houses.adm_esf_edw_house_daily_report_daac_001_df";
		private static String hdfsDest = "hdfs://s1-hadoop-test.esf.fdd:8020/user/qianjiali/data/edw_etl/dim_esf_edw_etl_source_target_map_df/";
	    private static String sqlDir = "http://s1-hadoop-test.esf.fdd:50070/webhdfs/v1/user/qianjiali/edw_houses/ADM/DDL/edw_houses.adm_esf_edw_house_daily_report_daac_001_df";
	    ...
	    public static void main(String[] args) throws IOException, ParseException,SemanticException 
	    {
	       ...
	       //	TxtFile.WriteTxtFile("file_name|"+"source_table|"+"target_table|"+"operate_type"+"join_type"+"insert_timestamp\r\n",resultTxtFile);
	      	try {
				 HDFSFileUtil hdfsFileUtil = new HDFSFileUtil(hdfsConfig);				
				
				Path path = new Path(hdfsDir);				
				List<Path> list = hdfsFileUtil.listFile(path);
				
				for(Path file:list){
					String filename = file.getName();
					String filePath = "HSQL" + file.getParent().toString().replace(hdfsDir, "");
					if(filename.endsWith(".sql") && filename.startsWith("edw_"))
					{
						String hdfsUrl = sqlDir+"/"+filename+"?user.name=yanggang&op=OPEN";
						String localUrl = localBase+"/"+filePath+"/"+filename;
						hdfsFileUtil.httpGetFile(hdfsUrl,localUrl);
					}
				}
